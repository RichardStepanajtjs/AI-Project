...
const getAllCompaniesWithoutEmbeddings = async () => {
    const { rows } = await pool.query('SELECT id, naam, kbonummer, postcode, gemeente, landcode, email, telefoonnummer, jobdomein FROM companies ORDER BY created_at DESC');
    return rows;
}
